...
def random_attack(x, epsilon, _obs_min_norm, _obs_max_norm):
    ...
    adv_x = preprocess_state(x.detach().clone())  # detach() then clone() is much efficient. Clone() also clone gradient graph. It is better to remove this step by firstly detach() it.
...
def critic_normal_attack(x, _policy, _q_func, epsilon, num_steps, step_size,
    ...
    assert isinstance(x, torch.Tensor), "input x must be tensor."
    ori_x = preprocess_state(x.detach().clone())                   # already normalized

    adv_x = ori_x.detach().clone()               # already normalized
...
def critic_mqd_attackV2(x, a, _policy, _q_func,gt_qval,qindex, epsilon, num_steps, step_size, _obs_min, _obs_max,
    ...
    # TODO: Optimized with PGD
        ...
        adv_x = ori_state_tensor.detach().clone()   
        noise = torch.zeros_like(adv_x).uniform_(-epsilon, epsilon)
        adv_x = adv_x + noise  
       
        for _ in range(num_steps):
            adv_x.requires_grad = True
            qval = _q_func(adv_x, a, "none")[qindex]     # Take q1

            cost = F.mse_loss(qval, gt_qval)

            grad = torch.autograd.grad(cost, adv_x, retain_graph=False, create_graph=False)[0]

            adv_x = adv_x + step_size * grad
            # clip into the upper and lower bounds
            adv_x = torch.max(adv_x, adv_lb)
            adv_x = torch.min(adv_x, adv_ub)
            # clip into the origin space
